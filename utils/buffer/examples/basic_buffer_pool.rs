...
fn main() {
    ...
    // Write data (12 bytes) into the buffer. Fills the first slot with 12 bytes, leaving 7
    // remaining slots open to fit a maximum of 20 more bytes worth of data for future writes
    ...
    assert_eq!(buffer_pool.len(), 12); // Buffer still owns data
    ...
    let expect_data = [67, 105, 97, 111, 44, 32, 109, 117, 110, 100, 111, 33];
    assert_eq!(data_slice.as_ref(), expect_data);
}
