...
/// Used by downstream role to set a custom trusted external Job Declarator.
///
/// The [`SetCustomMiningJob::token`] should provide the information for the upstream to authorize
/// the custom job that has been or will be negotiated between the Job Declarator and Pool.
///
/// Can be sent only on extended channel.
///
/// Previously exchanged [`SetupConnection::flags`] must contain `REQUIRES_WORK_SELECTION` flag
/// i.e., work selection feature was successfully negotiated.
///
/// [`SetupConnection::flags`]: crate::common_messages_sv2::SetupConnection::flags
...
/// Used by upstream as a response to accept the custom job request.
///
...
/// Used by upstream as a response to reject the custom job request.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct SetCustomMiningJobError<'decoder> {
    ...
    /// Request identifier set by the downstream role.
    pub request_id: u32,
    /// Reason why the mining job has been rejected.
    ///
    /// Possible errors:
    /// - invalid-channel-id
