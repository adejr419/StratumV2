...
/// Used by Downstream to notify an upstream about changes on the specified channel.
///
/// If a downstream performs device/connection aggregation (i.e. it is a proxy), it must send this
/// message when downstream channels change. This update can be debounced so that it is not sent
/// more often than once in a second (for a very busy proxy).
///
...
pub struct UpdateChannel<'decoder> {
    ...
    /// upstream node must reflect the downstreams’s request (and send appropriate SetTarget message).
    ///
...
/// Used by upstream to notify downstream about an error in the [`UpdateChannel`] message.
#[derive(Serialize, Deserialize, Debug, Clone)]
