...
/// Used by a downstream to request opening a standard mining channel with an upstream.
///
/// Opening a channel is necessary for almost all later communication, thus every downstream should
/// start its communication with an upstream role by opening a channel. The upstream role either
/// passes opening the channel further or has enough local information to handle channel opening on
/// its own (this is mainly intended for v1 proxies).
///
/// If the upstream responds with a [`OpenStandardMiningChannelSuccess`] message, the downstream
/// should open channel(s) on the connection, otherwise if no channels are opened within a
/// reasonable period, the upstream should close the connection for inactivity.
///
/// Downstreams must also communicate information about their hashing power in order to receive
...
/// Used by upstream as a response for a request to open standard channel from downstream.
...
pub struct OpenStandardMiningChannelSuccess<'decoder> {
    ...
    /// served by the upstream role for a set of standard channels that belong to the same group.
    #[cfg_attr(feature = "with_serde", serde(borrow))]
...
/// Used by a downstream to request opening an extended mining channel with an upstream role.
///
/// Similar to [`OpenStandardMiningChannel`] but requests to open an extended channel instead of
/// standard channel. The main difference between standard and extended channels is that the
/// extranonce size is not fixed for extended channels and can be set by the upstream role based on
/// the [`OpenExtendedMiningChannel::min_extranonce_size`] field.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq)]
...
/// Used by upstream as a response for a request to open extended channel from downstream.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq)]
pub struct OpenExtendedMiningChannelSuccess<'decoder> {
    /// Specified by downstream role aand should be extracted from the corresponding
