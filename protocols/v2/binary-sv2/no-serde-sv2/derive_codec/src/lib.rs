...
//! ## Overview
//!
//! These macros parse struct definitions to produce code that supports efficient and type-safe serialization
//! and deserialization. Each field within a struct is processed based on its type and associated generics, allowing
//! for custom encoding schemes and alignment with protocol requirements. Additionally, the macros enable flexible
//! handling of lifetimes and static references to ensure compatibility across different use cases.
//!
//! ## Available Macros
//!
//! - **`Encodable`**: Automatically implements encoding logic, converting a struct's fields into a binary format.
//!   - **Attributes**: `#[already_sized]` (optional) to specify that a struct's size is fixed at compile-time.
//!   - **Generated Traits**: `EncodableField` (field-by-field encoding) and `GetSize` (size calculation).
//!
//! - **`Decodable`**: Automatically implements decoding logic, allowing a struct to be reconstructed from binary data.
//!   - **Generated Methods**: `get_structure` (defines field structure) and `from_decoded_fields` (builds the struct from decoded fields).
//!
//! ## Internal Structure
//!
//! ### `is_already_sized`
//! Checks if the `#[already_sized]` attribute is present on the struct, allowing certain optimizations in generated code for fixed-size structs.
//!
...
//! The `Encodable` macro generates an `EncodableField` implementation by serializing each field, while `Decodable`
//! constructs the struct from binary data. Both macros provide support for structs with or without lifetimes, ensuring
//! versatility in applications that require efficient, protocol-level data handling.
...
/// byte stream, including its structure, field decoding, and a method for creating a static version.
///
